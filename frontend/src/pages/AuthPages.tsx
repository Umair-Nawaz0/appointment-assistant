...
export function SignupPage(){
  ...
  const submit=async(e:FormEvent)=>{e.preventDefault();setError('');setBusy(true);try{const result=await api<{message:string;email:string}>('/auth/signup',{method:'POST',body:JSON.stringify({...form,industry:form.industry||undefined})});navigate('/verify-email',{state:{email:result.email}});}catch(err){setError(err instanceof Error?err.message:'Unable to create workspace.');}finally{setBusy(false)}};
...
export function VerifyEmailPage(){
  const location=useLocation();const initial=(location.state??{}) as {email?:string};const [email,setEmail]=useState(initial.email??'');const [code,setCode]=useState('');const [status,setStatus]=useState('');const [error,setError]=useState('');const [busy,setBusy]=useState(false);
  ...
  const resend=async()=>{setBusy(true);setError('');try{const result=await api<{message:string}>('/auth/resend-verification',{method:'POST',body:JSON.stringify({email})});setStatus(result.message);}catch(err){setError(err instanceof Error?err.message:'Unable to resend code.');}finally{setBusy(false)}};
  return <AuthShell eyebrow="One last step" title={status?'Email verified':'Verify your business email'} subtitle="Enter the six-digit code sent to your email.">{status&&status.startsWith('Email verified')?<div className="auth-result"><MailCheck size={42}/><Notice>{status}</Notice><Link className="button" to="/login">Continue to sign in</Link></div>:<form className="auth-form" onSubmit={submit}>{error&&<Notice kind="error">{error}</Notice>}{status&&<Notice>{status}</Notice>}<Field label="Business email"><Input type="email" value={email} onChange={e=>setEmail(e.target.value)} required/></Field><Field label="Verification code"><Input value={code} onChange={e=>setCode(e.target.value.replace(/\D/g,'').slice(0,6))} inputMode="numeric" pattern="[0-9]{6}" autoComplete="one-time-code" placeholder="Enter 6-digit code" required/></Field><Button disabled={busy||code.length!==6}>{busy?'Checking…':'Verify email'}</Button><button className="dev-link" type="button" disabled={busy||!email} onClick={()=>void resend()}>Send a new code</button></form>}</AuthShell>;
}

export function ForgotPasswordPage(){
  const navigate=useNavigate();const [email,setEmail]=useState('');const [result,setResult]=useState<{message:string}|null>(null);const [error,setError]=useState('');const [busy,setBusy]=useState(false);
  const submit=async(e:FormEvent)=>{e.preventDefault();setBusy(true);setError('');try{setResult(await api<{message:string}>('/auth/forgot-password',{method:'POST',body:JSON.stringify({email})}));}catch(err){setError(err instanceof Error?err.message:'Request failed.');}finally{setBusy(false)}};
  return <AuthShell eyebrow="Account recovery" title="Reset your password" subtitle="Enter your email and we’ll send a six-digit reset code."><form className="auth-form" onSubmit={submit}>{error&&<Notice kind="error">{error}</Notice>}{result&&<Notice>{result.message}</Notice>}<Field label="Business email"><Input type="email" value={email} onChange={e=>setEmail(e.target.value)} required/></Field><Button disabled={busy}>{busy?'Sending…':<>Send reset code <ArrowRight size={17}/></>}</Button>{result&&<button className="dev-link" type="button" onClick={()=>navigate('/reset-password',{state:{email}})}>Enter reset code</button>}<p className="form-switch"><Link to="/login">Back to sign in</Link></p></form></AuthShell>;
}

export function ResetPasswordPage(){
  const location=useLocation();const initial=(location.state??{}) as {email?:string};const [email,setEmail]=useState(initial.email??'');const [code,setCode]=useState('');const [password,setPassword]=useState('');const [done,setDone]=useState(false);const [error,setError]=useState('');const [busy,setBusy]=useState(false);
  ...
  return <AuthShell eyebrow="Choose a new password" title={done?'Password updated':'Secure your business'} subtitle={done?'Your business workspace is ready.':'Enter the reset code and choose a unique password.'}>{done?<div className="auth-result"><CheckCircle2 size={44}/><Link className="button" to="/login">Sign in</Link></div>:<form className="auth-form" onSubmit={submit}>{error&&<Notice kind="error">{error}</Notice>}<Field label="Business email"><Input type="email" value={email} onChange={e=>setEmail(e.target.value)} required/></Field><Field label="Reset code"><Input value={code} onChange={e=>setCode(e.target.value.replace(/\D/g,'').slice(0,6))} inputMode="numeric" pattern="[0-9]{6}" autoComplete="one-time-code" placeholder="Enter 6-digit code" required/></Field><Field label="New password" hint="At least 10 characters with upper, lower, and a number."><PasswordInput value={password} onChange={setPassword} autoComplete="new-password"/></Field><Button disabled={busy||code.length!==6}>{busy?'Updating…':<><KeyRound size={17}/> Update password</>}</Button></form>}</AuthShell>;
}
